...
/*   Updated: 2023/01/05 10:31:36 by abouabra         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!ft_atoi(tmp[j]) && !ft_is_number(tmp[j]))
	{
		ft_dprintf(2, "Error\nArgument is not number: %s\n", tmp[j]);
		free_char_array(tmp);
		free(final);
		return (0);
	}
...
long	*extract_tmp_arr(t_args *vars, int ac, char **av)
{
	...
	final = ft_calloc(1, (vars->args_counter + 1) * sizeof(long));
	if (!final)
		return (0);
	i = 0;
	x = 0;
...
int	check_is_duplicated_num(t_args *vars)
{
	...
	while (++i < vars->args_counter)
	{
		...
		while (++j < vars->args_counter)
		{
			...
			{
				ft_dprintf(2, "Error\nDuplicated number Detected\n");
				return (0);
			}
		}
		if (vars->tmp_arr[i] > INT_MAX || vars->tmp_arr[i] < INT_MIN)
		{
			ft_dprintf(2, "Error\nArgument larger than \"int\" range\n");
			return (0);
		}
	}
	return (1);
